...
"---------------------------------------------------------------------------
"カスタム
"set   window=100,40                 " Window サイズを 100x40 にする
...
"colorscheme custom " (GUI使用時)

map   <M-Space> :simalt ~<CR>
nnoremap <S-Insert> "*P
" 通常モードの<S-Insert>を再定義する(貼り付けにする)
winpos 0 326                        " ウィンドウの開始位置を固定とする
command! -nargs=0 Defwinpos winpos 0 326     " defaultポジションに移動関数
command! -nargs=0 WinposDefault winpos 0 326     " defaultポジションに移動関数
set guioptions-=a		    " Visualモードでテキスト選択時に自動でクリップボードに
				    " 入れないようにする(a)
set guioptions-=T		    " ToolBarを表示しないようにする(T)
set guioptions-=L		    " 左のスクロールバーを無くす(L)
"set guioptions=gmrLta              " gmrLtaからToolBarを抜かしたもの
"set guioptions=gmrt                " 上記より、VISUALモードでテキスト選択時に自動で
				    " クリップボードに入れないように変更
set clipboard=unnamed               " yy等で直接クリップボードに入れる
"set guifont=ＭＳ_ゴシック:h10.5:cSHIFTJIS
if has('win32')
  set guifont=MS_Gothic:h10.5:cSHIFTJIS
endif
"set guifont=Consolas:h10.0:cSHIFTJIS
"set guifont=MeiryoKe_Gothic:h10.5:cSHIFTJIS
"set guifont=MeiryoKe_Gothic:h10:cSHIFTJIS
"set guifont=MeiryoKe_UIGothic:h10.5:cSHIFTJIS
"set guifont=MeiryoKe_PGothic:h9:cSHIFTJIS

"colorscheme darkblue              " 画面をdarkblueにする
"colorscheme torte              " 画面をtorteにする

"colorscheme default " (GUI使用時)
"highlight CursorIM guibg=Purple   " 変換モード時のカーソルの色を紫に
"highlight Cursor guibg=Green guifg=NONE   " 通常時のカーソルの色を緑に

"File、Toolsが二重なってしまうのを防ぐために定義"
"source $VIM/vimfiles/mru.vim
"source $HOME/vimfiles/grep.vim
"source $HOME/vimfiles/taglist.vim
"source $HOME/vimfiles/blockdiff.vim
"source $VIM/vimfiles/bookmarks.vim
...
if has('win32')
  ...
  "アンダーバーつき
  "highlight PmenuSel ctermbg=13 guibg=lightmagenta gui=undercurl guisp=Red term=reverse
  "highlight PmenuSbar ctermbg=13 guibg=lightmagenta gui=undercurl guisp=Red term=reverse
  "アンダーバーなし
  highlight PmenuSel ctermbg=13 guibg=lightmagenta 
  highlight PmenuSbar ctermbg=13 guibg=lightmagenta
endif
if has('gui_macvim')
